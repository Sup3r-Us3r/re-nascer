          ...
          <div key={sale.id} className="flex items-center justify-between rounded-lg border border-border p-4">
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <h3 className="font-semibold text-foreground">{sale.clientName}</h3>
              </div>
              <p className="text-sm text-muted-foreground">Produto: {sale.productType}</p>
              <p className="text-sm text-muted-foreground">
                Data: {new Date(sale.date).toLocaleDateString('pt-BR')}
              </p>
              <div className="mt-2 flex gap-4">
                <span className="text-sm font-medium text-foreground">⚖️ {sale.weight} kg</span>
